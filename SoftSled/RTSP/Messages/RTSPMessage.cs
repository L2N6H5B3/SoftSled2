namespace Rtsp.Messages
{
    ...
    public class RtspMessage : RtspChunk
    {
        ...
        public void AddHeader(string line)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Invalid Header received : -{line}-");
            }
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Commande : {Command}");
            foreach (KeyValuePair<string, string> item in _headers)
            {
                System.Diagnostics.Debug.WriteLine($"Header : {item.Key}: {item.Value}");
            }

            if (Data.Length > 0)
            {
                System.Diagnostics.Debug.WriteLine($"Data :-{ASCIIEncoding.ASCII.GetString(Data)}-");
            }
        }
