...
namespace SoftSled.Components.RTSP {
    class RTSPClient {
        ...
        private string AcceptLanguageHeader = "Accept-Language: en-us, *;q=0.1";
        private string BufferInfoHeader = "Buffer-Info.dlna.org: dejitter=6624000;CDB=6553600;BTM=0;TD=2000;BFR=0";
        private string UserAgentHeader = "User-Agent: MCExtender/1.50.X.090522.00"; // Assume Xbox 360?
        //private string UserAgentHeader = "User-Agent: MCExtender/1.0.0.0"; // Linksys Extender (Doesn't support H.264?)
        ...
        //private string SupportedHeader = "Supported: dlna.announce, dlna.rtx-dup";
        ...
        string[] valid_audio_codecs = { "PCMA", "PCMU", "AMR", "MPA", "MPEG4-GENERIC", "VND.MS.WM-MPA", "vnd.ms.wm-AC3", "X-WMF-PF" /* for aac */};
        ...
        public void Connect(string url, RTP_TRANSPORT rtp_transport, MEDIA_REQUEST media_request = MEDIA_REQUEST.VIDEO_AND_AUDIO) {
            ...
            if (rtp_transport == RTP_TRANSPORT.UDP) {
                Debug.WriteLine("UDP Transport");
                //video_udp_pair = new Rtsp.UDPSocket(50000, 51000); // give a range of 500 pairs (1000 addresses) to try in case some address are in use
                //video_udp_pair.DataReceived += Rtp_VideoDataReceived;
                //video_udp_pair.Start(); // start listening for data on the UDP ports
                //audio_udp_pair = new Rtsp.UDPSocket(50000, 51000); // give a range of 500 pairs (1000 addresses) to try in case some address are in use
            ...
            options_message.AddHeader(AcceptLanguageHeader);
            options_message.AddHeader(SupportedHeader);
            options_message.AddHeader(UserAgentHeader);
        ...
        public void Pause() {
            if (rtsp_client != null) {
                ...
                pause_message.AddHeader(AcceptLanguageHeader);
                pause_message.AddHeader(SupportedHeader);
                pause_message.AddHeader(UserAgentHeader);
        ...
        public void Rtp_VideoDataReceived(object sender, Rtsp.RtspChunkEventArgs e) {
            ...
            if (data_received.Channel == video_data_channel || data_received.Channel == audio_data_channel) {
                ...
                if (rtp_extension == 1) {
                    Debug.WriteLine("RTP has Extension!");
                ...
                Debug.WriteLine($"RTP Payload Type '{rtp_payload_type}' is Codec '{wmfPayloadDataDict[rtp_payload_type].Codec}' ClockRate '{wmfPayloadDataDict[rtp_payload_type].ClockRate}' EncodingParameters {wmfPayloadDataDict[rtp_payload_type].EncodingParameters} FormatParameter {wmfPayloadDataDict[rtp_payload_type].FormatParameter}");
        ...
        public void Rtp_AudioDataReceived(object sender, Rtsp.RtspChunkEventArgs e) {
            ...
            if (data_received.Channel == video_data_channel || data_received.Channel == audio_data_channel) {
                ...
                //Debug.WriteLine("RTP Header Data"
                //                   + " V=" + rtp_version
                //                   + " P=" + rtp_padding
                //                   + " X=" + rtp_extension
                //                   + " CC=" + rtp_csrc_count
        ...
        private void Rtsp_MessageReceived(object sender, Rtsp.RtspChunkEventArgs e) {
            ...
            if (message.OriginalRequest != null && message.OriginalRequest is Rtsp.Messages.RtspRequestOptions) {
                ...
                if (keepalive_timer == null) {
                    ...
                    describe_message.AddHeader(AcceptLanguageHeader);
                    describe_message.AddHeader(SupportedHeader);
                    describe_message.AddHeader(UserAgentHeader);
            ...
            if (message.OriginalRequest != null && message.OriginalRequest is RtspRequestDescribe) {
                ...
                // Examine the SDP
                System.Diagnostics.Debug.WriteLine("Describe Response:");
                ...
                for (int x = 0; x < sdp_data.Medias.Count; x++) {
                    ...
                    if (audio || video) {
                            ...
                            if (attrib.Key.Equals("rtpmap")) {
                                ...
                                if (video && Array.IndexOf(valid_video_codecs, rtpmap.EncodingName.ToUpper()) >= 0) {
                                    if (wmfPayloadDataDict.ContainsKey(rtpmap.PayloadNumber)) {
                                        ...
                                        wmfPayloadDataDict[rtpmap.PayloadNumber].ClockRate = rtpmap.ClockRate;
                        ...
                        if (rtp_transport == RTP_TRANSPORT.UDP) {
                            ...
                            if (video) {
                                video_data_channel = 50023;
                                video_rtcp_channel = 50024;
                                rtp_port = 50023;
                                rtcp_port = 50024;
                            ...
                            if (audio) {
                                audio_data_channel = 50025;
                                audio_rtcp_channel = 50026;
                                rtp_port = 50025;
                                rtcp_port = 50026;
                        ...
                        setup_message.AddHeader(AcceptLanguageHeader);
                        setup_message.AddHeader(BufferInfoHeader);
                        setup_message.AddHeader(SupportedHeader);
                        setup_message.AddHeader(UserAgentHeader);
                        //setup_message.AddHeader("X-Playlist-Gen-Id: 0");
            ...
            if (message.OriginalRequest != null && message.OriginalRequest is RtspRequestSetup) {
                ...
                // Log Number of found Streams
                Debug.WriteLine($"Found {wmfPayloadDataDict.Keys.Count} streams.");
                ...
                if (setup_messages.Count > 0) {
                    ...
                    next_setup.AddHeader(AcceptLanguageHeader);
                    next_setup.AddHeader(BufferInfoHeader);
                    next_setup.AddHeader(SupportedHeader);
                    next_setup.AddHeader(UserAgentHeader);
            ...
            if (message.OriginalRequest != null && message.OriginalRequest is Rtsp.Messages.RtspRequestPlay) {
                ...
                // Examine the SDP
                Debug.WriteLine("Play Response:");
                Debug.WriteLine(System.Text.Encoding.UTF8.GetString(message.Data));
        ...
        void Timer_Elapsed(object sender, System.Timers.ElapsedEventArgs e) {
            ...
            if (server_supports_get_parameter) {
                ...
                getparam_message.AddHeader(AcceptLanguageHeader);
                getparam_message.AddHeader(SupportedHeader);
                getparam_message.AddHeader(UserAgentHeader);
