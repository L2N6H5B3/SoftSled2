...
namespace Rtsp
{
    ...
    public class H264Payload
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("RTP Data comprised of " + rtp_payloads.Count + " rtp packets");
            ...
            for (int payload_index = 0; payload_index < rtp_payloads.Count; payload_index++)
            {
                ...
                else if (nal_header_type == 24)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("H264 Aggregate Packet processing error");
                    }
                }
                else if (nal_header_type == 25)
                {
                    System.Diagnostics.Debug.WriteLine("Agg STAP-B not supported");
                    stap_b++;
                }
                else if (nal_header_type == 26)
                {
                    System.Diagnostics.Debug.WriteLine("Agg MTAP16 not supported");
                    mtap16++;
                }
                else if (nal_header_type == 27)
                {
                    System.Diagnostics.Debug.WriteLine("Agg MTAP24 not supported");
                    mtap24++;
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Frag FU-B not supported");
                    fu_b++;
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine("Unknown NAL header " + nal_header_type + " not supported");
                }

            }

            // Output some statistics
            System.Diagnostics.Debug.WriteLine("Norm=" + norm + " ST-A=" + stap_a + " ST-B=" + stap_b + " M16=" + mtap16 + " M24=" + mtap24 + " FU-A=" + fu_a + " FU-B=" + fu_b);
